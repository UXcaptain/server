...
const ParticipateContent = () => {
  ...
  // Step 1: Validate analysis ID and create entry
  const handleSubmitId = async (submittedAnalysisId) => {
    ...
    try {
      ...
      // Create analysis entry and fetch data in one call
      ...
      if (err?.response?.status === 404) {
        ...
        setError(err?.response?.data?.message || "Ocurrió un error al crear la entrada de análisis");
      }
      setAnalysisId(null);
    } finally {
